	...
	union() {
		difference() {
			intersection() {
				...
				difference() {
					translate([k_inner_height/3, 0, 0]) 
						cylinder(r=k_outer_radius, h=height+1, center=true);
					translate([k_outer_radius + 2*wall_thickness, 0, 0]) 
						cylinder(r=k_inner_height/3+1, h=height+2, center=true);
